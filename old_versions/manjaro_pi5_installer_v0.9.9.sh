#!/usr/bin/env bash
#
# Manjaro ARM Pi 5 Installer with KDE Plasma Support - FIXED
...
ui_select_display_manager() {
    ...
    while true; do
        DISPLAY_MANAGER=$(dialog_input "Display Manager" \
            --menu "Select display manager:" 15 60 3 \
...
load_profile_packages() {
    ...
    # For KDE, use custom package list since upstream removed KDE support
    if [[ "$EDITION" =~ ^kde- ]]; then
...
install_profile_packages() {
    ...
    set -- base manjaro-system manjaro-release systemd systemd-libs \
        sudo $PKG_SHARED $PKG_EDITION $PKG_DEVICE
...
First boot may take a few minutes to initialize." 18 70
}
