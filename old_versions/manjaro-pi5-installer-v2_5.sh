...
# Version: 2.5 - Wi-Fi Configuration + Fixed Xorg Auto-Detection
#
# Changes in v2.5:
...
install_pi5_hardware_support() {
    ...
    log INFO "Installing Pi 5-specific packages"
    ...
    $NSPAWN "$TMPDIR/root" pacman -S --noconfirm --needed \
        ...
        linux-rpi5 \
        linux-rpi5-headers \
        uboot-tools 2>&1 | tee -a "$TMPDIR/install.log"
    ...
    $NSPAWN "$TMPDIR/root" pacman -S --noconfirm --needed \
        ...
        alsa-utils 2>&1 | tee -a "$TMPDIR/install.log" || true
...
main() {
    ...
    mkdir -p "$TMPDIR"
    log INFO "=== Manjaro ARM Pi 5 Installer v2.5 Started ==="
